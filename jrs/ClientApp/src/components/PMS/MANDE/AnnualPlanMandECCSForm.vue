                ...
                <td :class="'descClass'">
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                      <span v-on="on">{{ item.PMS_CCS_DESC | subStr }}</span>
                    </template>
                    <span>{{ item.PMS_CCS_DESC }}</span>
                  </v-tooltip>
                </td>
                <td :class="'descClass'">
...
import Indicators from "../CROSSCUTTING/AnnualPlanCCSIndicatorForm.vue";
...
*/
    checkboxUpdated(newValue: any, index: number, rel_id: number) {
      ...
      for (i = 0; i < count; i++) {
        ...
        if (newValue) {
          localThis.formData.selectedCCSId = localThis.ccsList[index].PMS_CCS_KEY_ID;
