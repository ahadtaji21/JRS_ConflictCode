  ...
  <div>
    ...
    >
      <pms-outp-ind-det-form
        @closeCCSDetDialoge="closeDialog"
        :formData="formData"
        @closeDialogeAndSaveCCSInd="closeDialog"
  ...
  methods: {
    ...
    getIndicators() {
      ...
      let selectPayload: GenericSqlSelectPayload = {
        viewName: "PMS_VI_CCS_INDICATOR_LIST",
        colList: null,
        whereCond: `Dimension='Cross-cutting service'`, // AND IMS_LANGUAGE_LOCALE='${i18n.locale}'`,
        orderStmt: "PMS_CCS_IND_QUESTION",
      };
              ...
              if (localThis.indicatorsAlreadyAdded != undefined) {
                if (
                  localThis.indicatorsAlreadyAdded.filter(
                    (itemL: any) => itemL.PMS_CCS_IND_ID == item.PMS_CCS_IND_ID
                  ).length == 0
                ) {
  ...
  watch: {
    selectedItm(to: Array<any>, from: Array<any>) {
      ...
      if (to.length > 0) {
        ...
        localThis.formData.ccsId = localThis.selectedCCSId;
        localThis.formData.ccsIndicatorId = to[0].PMS_CCS_IND_ID;
        localThis.formData.QUESTION = to[0].PMS_CCS_IND_QUESTION;
        localThis.formData.INDICATOR = to[0].PMS_CCS_IND;
