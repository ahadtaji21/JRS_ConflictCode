              ...
              <tr v-for="(item, index) in items" :key="item.ID">
                <td :class="'descClass'">
                  <v-checkbox
                    ...
                    @change="checkboxUpdated($event, index, item.ID, item.PMS_OVG_KEY_ID)"
                  ></v-checkbox>
                </td>
                <td :class="'descClass'">
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                      <span v-on="on">{{ item.PMS_OVERALL_GOAL_DESC | subStr }}</span>
                    </template>
...
import Indicators from "../OVERALLGOAL/AnnualPlanOverallGoalIndicatorForm.vue";
...
*/
    checkboxUpdated(newValue: any, index: number, rel_id: number, ovg_id: number) {
      let localThis: any = this as any;
      ...
      for (i = 0; i < count; i++) {
        ...
        if (newValue) {
          localThis.formData.selectedOvgId = ovg_id; //localThis.overallList[index].PMS_OVG_KEY_ID;
