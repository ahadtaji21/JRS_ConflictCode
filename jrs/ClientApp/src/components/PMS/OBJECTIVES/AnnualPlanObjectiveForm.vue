                ...
                >
                  <template v-slot:activator="{ on }">
                    <v-btn
                      ...
                      @click="clearEditItem"
                    >
                      <v-icon>mdi-plus-circle-outline</v-icon>ADD TARGET
                    </v-btn>
                  </template>
                  <pms-addobj-form
              ...
              <tr v-for="item in items" :key="item.ID" style="height: 10px">
                ...
                <td
                  v-if="
                    module == 'IMP' &&
                    item.ACTIVATION_STATE > 0 &&
                    item.ACTIVATION_STATE < item.SERVICE_NUMBER > 0
                  "
                  class="tablerow"
                >
                  <v-tooltip bottom>
                ...
                <td
                  v-if="
                    module == 'IMP' &&
                    item.ACTIVATION_STATE == item.SERVICE_NUMBER
                  "
                  class="tablerow"
                >
                  <v-tooltip bottom>
...
import AddObjective from "./AnnualPlanObjectiveMainDataForm.vue";
// import Narrative from "../PMS/AnnualPlanNarrative.vue";
// import Services from "../PMS/AnnualPlanServiceForm.vue";
import ObjDetails from "./AnnualPlanObjectiveDetailsForm.vue";
...
export default Vue.extend({
  ...
  methods: {
    ...
    async insert(payload: GenericSqlPayload) {
      let localThis = this as any;
      const retvalue = await localThis.execSPCall(payload).then((res: any) => {
        return res;
      });
    },
    ...
    getOBJ() {
      ...
      return localThis
        ...
        .then((res: any) => {
          ...
          localThis.objectiveListOrig = [...(res.table_data ? res.table_data : [])];
          if (localThis.tmpSelectedItem && localThis.tmpSelectedItem.length == 1) {
            let i = 0;
            for (i = 0; i < localThis.objectiveList.length; i++) {
              if (localThis.objectiveList[i].ID == localThis.tmpSelectedItem.ID) {
                localThis.tmpSelectedItem = [];
                localThis.tmpSelectedItem.push(
                  JSON.parse(JSON.stringify(localThis.objectiveList[i]))
                );
  ...
  mounted() {
    ...
    EventBus.$on("reloadObj", (data: any) => {
      localThis.reloadObjectives(data);
    });
  },
});
</script>
