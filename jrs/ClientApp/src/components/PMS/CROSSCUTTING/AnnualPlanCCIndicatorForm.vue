  ...
  <div>
          ...
          <div class="d-inline-flex align-center primary lighten-2 jrs-table-top">
            ...
            >
              ...
              <search-table
                ...
                :selectedCCId="formData.selectedCCId"
                :selectedCCKeyId="formData.selectedCCKeyId"
                :indicatorsAlreadyAdded="indicatorList"
                :key="Math.ceil(Math.random() * 1000)"
                @UpdateCCCIndicators="updateCCCIndicators"
              ></search-table>
            </v-dialog>
        ...
        <template v-slot:body="{ items }">
          <tbody style="border: solid">
            <tr v-for="item in items" :key="item.ID" style="height: 10px">
              <td class="tablerow">{{ item.PMS_CC_IND }}</td>
              <!-- <td class="tablerow">{{ item.PMS_CC_IND_STANDARD }}</td>
              <td class="tablerow">{{ item.PMS_CC_IND_PERIODICITY }}</td>
              <td class="tablerow">{{ item.PMS_CC_IND_DISAGGREGATED }}</td> -->
              <td class="tablerow">{{ item.PMS_CC_IND_TECHNIQUE }}</td>
              <td class="tablerow">{{ item.PMS_AP_CC_END_DATE | formatDate }}</td>
    ...
    >
      <pms-outp-ind-det-form
        @closeCCDetDialoge="closeOutpDialog"
        @closeDialogeAndSaveCCInd="closeOutpDialogAndSave"
...
import SearchTable from "./AnnualPlanCCIndicatorSearch.vue";
...
/* localThis.localFormData.selectedProcessId = localThis.selectedProcessId;
      ...
      localThis.localFormData.selectedCCId = localThis.selectedCCId;
...
export default Vue.extend({
  ...
  methods: {
    ...
    UpdateItems(item: any) {
      ...
      let annal_plan_id = ap.annal_plan_id;
      let i: number = 0;
      let j: number = 0;
      for (i = 0; i < item.length; i++) {
        payLoadD.PMS_CC_IND_REL_ID = 0;
        payLoadD.PMS_AP_ID = annal_plan_id;
        payLoadD.PMS_CC_ID = localThis.formData.selectedCCKeyId;
        payLoadD.PMS_IND_ID = item[i].PMS_CC_IND_ID;

        let savePayload: GenericSqlPayload = {
          spName: "PMS_SP_INS_CC_INDICATOR",
    ...
    getOutputIndicators() {
      ...
      let wherecond: string = `PMS_CC_KEY_ID = ${localThis.formData.selectedCCKeyId}`;
    ...
    editIndicator(item: any) {
      ...
      localThis.localFormData.ccIndicatorRelId = item.PMS_CC_IND_REL_ID;
      localThis.localFormData.ccId = localThis.formData.selectedCCId;
      localThis.localFormData.ccIndicatorId = item.PMS_CC_IND_ID;
      localThis.localFormData.QUESTION = item.PMS_CC_IND_QUESTION;
      localThis.localFormData.INDICATOR = item.PMS_CC_IND;
      localThis.localFormData.VALUE = item.VALUE;
      localThis.localFormData.END_DATE = item.PMS_AP_CC_END_DATE;
      localThis.localFormData.STANDARD = item.PMS_CC_IND_STANDARD;
      localThis.localFormData.TECHNIQUE = item.PMS_CC_IND_TECHNIQUE;
      localThis.localFormData.DISAGGREGATED = item.PMS_CC_IND_DISAGGREGATED;
      localThis.localFormData.PERIODICITY = item.PMS_CC_IND_PERIODICITY;
      localThis.localFormData.VALUE = item.VALUE;
    ...
    deleteItem(item: any) {
      ...
      this.$confirm(msg).then((res: any) => {
        if (res) {
          let payLoadD = {} as any;
          payLoadD.PMS_CC_IND_REL_ID = item.PMS_CC_IND_REL_ID;

          let savePayload: GenericSqlPayload = {
            spName: "PMS_SP_DEL_CC_INDICATOR",
            actionType: SqlActionType.NUMBER_0,
            jsonPayload: JSON.stringify(payLoadD),
          };
          localThis
  ...
  mounted() {
    ...
    if (localThis.formData.selectedCCId > 0) {
      localThis.getOutputIndicators();
    }

    EventBus.$on("closeDialogeAndSaveCCInd", (data: any) => {
      localThis.showIndicatorDetails = false;
      if (localThis.formData.selectedCCId > 0) {
        localThis.editMode = false;
        localThis.getOutputIndicators();
