          ...
          <v-row>
            <v-col :cols="$vuetify.breakpoint.xsOnly ? 12 : 6">
              <jrs-multi-select-table
                v-model="formData.STRAT_SUPP_SERV"
                :label="$t('datasource.pms.annual-plan.objectives.s-s-s')"
                :dataSource="'PMS_VI_CONSTRUCTION_DEF_DIMENSION_SSS'"
...
export default Vue.extend({
  ...
  methods: {
    ...
    save: function () {
      ...
      // payLoadD.STRAT_SUPP_SERV_ID =
      //   localThis.formData.STRAT_SUPP_SERV[0].PMS_CONSTRUCTION_DEF_ID;
      // payLoadD.PMS_SSS_KEY_ID = localThis.formData.selectedOvgId;

      if (localThis.formData.STRAT_SUPP_SERV == undefined) return;
      this.$confirm(msg).then(async (res) => {
        if (res) {
          let i: number = 0;

          for (i = 0; i < localThis.formData.STRAT_SUPP_SERV.length; i++) {
            let payLoadD: any = {};
            payLoadD.ANNUAL_PLAN_ID = localThis.annual_plan_id;
            payLoadD.STRAT_SUPP_SERV_ID =
              localThis.formData.STRAT_SUPP_SERV[i].PMS_CONSTRUCTION_DEF_ID;
            payLoadD.PMS_SSS_KEY_ID = localThis.formData.selectedOvgId;

            let savePayload: GenericSqlPayload = {
              spName: "PMS_SP_INS_UPD_ANNUAL_PLAN_STRAT_SUPP_SERV",
    ...
    resetOverall() {
      ...
      localThis.formData.STRAT_SUPP_SERV = [];
      localThis.dataSourceCondition = "PMS_DIMENSION='SStrategic & support process'";
  ...
  beforeMount() {
    ...
    localThis.dataSourceCondition = "PMS_DIMENSION='Strategic & support process'";
  },
});
</script>
