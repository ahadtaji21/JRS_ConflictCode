          ...
          >
            <template v-slot:top>
              <div class="d-inline-flex align-center primary lighten-2 jrs-table-top">
                <h4>
                  {{ $tc("datasource.pms.annual-plan.objectives.activated-proc", 2) }}
                </h4>
                <v-spacer></v-spacer>
            ...
            <template v-slot:body="{ items }">
              <tr v-for="item in items" :key="item.PMS_ACTIVITY_OUTPUT_REL_ID">
                <td :class="'descClass'">
                  <v-tooltip bottom>
                ...
                <td :class="'descClass'">
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                      <span v-on="on">{{ item.PMS_ACTIVATION_DATE | formatDate }}</span>
                    </template>
                    <span>{{ item.PMS_ACTIVATION_DATE }}</span>
                  </v-tooltip>
                </td>
                <td :class="'descClass'">
                  <v-tooltip bottom>
                ...
                <td :class="'descClass'">
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                      <span v-on="on">{{ item.PMS_COLLECTION_TYPE | subStr }}</span>
                    </template>
                    <span>{{ item.PMS_COLLECTION_TYPE }}</span>
                  </v-tooltip>
                </td>
                <td :class="'descClass'">
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                      <span v-on="on">{{ item.TECHNIQUE | subStr }}</span>
                    </template>
...
*/
    ...
    getAnnualPlanObjectives() {
      ...
      let i: number = 0;
      let j: number = 0;
      let view: string = "PMS_VI_ANNUAL_PLAN_ACTIVATED_PROCESS_OVERVIEW";
      let wherecond: string = `AP_ID = ${localThis.selectedAnnualPlanId} `;
      if (localThis.versioned > 0) {
        view = "PMS_VI_ANNUAL_PLAN_OUTP_OVERVIEW";
        ...
        (res: any) => {
          ...
          if (res.table_data) {
            ...
            res.table_data.forEach((item: any) => {
              ...
              objItem.PMS_ACTIVATION_DATE = item.ACTIVATION_DATE;
              objItem.PMS_COLLECTION_TYPE = item.COLLECTION_TYPE;
    ...
    setupHeaders() {
      ...
      let tmpOUTC: JrsHeader[] = [
        ...
        {
          text: this.$i18n.t("general.date-activation").toString(),
          value: "PMS_ACTIVATION_DATE",
          align: "center",
          divider: true,
          width: descWidth,
        },
        ...
        {
          text: this.$i18n.t("general.collection-type").toString(),
          value: "PMS_COLLECTION_TYPE",
          align: "center",
          divider: true,
          width: descWidth,
        },
        {
          text: this.$i18n.t("pms.technique").toString(),
          value: "TECHNIQUE",
          align: "center",
          divider: true,
          width: descWidth,
