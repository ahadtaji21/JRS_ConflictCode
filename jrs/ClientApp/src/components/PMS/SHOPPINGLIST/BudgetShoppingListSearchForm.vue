  ...
  <div>
    <v-card>
      ...
      <v-card-text>
        <v-data-table
          :headers="headers"
          :items="shoppinglistList"
          item-key="PMS_SHOPPING_LIST_ID"
          multi-sort
          :search="tableFilter"
          :items-per-page="10"
...
export default Vue.extend({
  ...
  methods: {
    ...
    getItems() {
      ...
      localThis.shoppinglistList = [];
      let itemsAlreadyInserted: any = [];
      localThis.shoppinglistListOrig = [];
      let i: number = 0;
      let j: number = 0;
      let selectPayload: GenericSqlSelectPayload = {
        viewName: "PMS_SHOPPING_LIST",
        colList: null,
        whereCond: `PMS_BUDGET_RESOURCE_ID=${localThis.selectedResId}`, // AND IMS_LANGUAGE_LOCALE='${i18n.locale}'`,
        orderStmt: "PMS_SHOPPING_LIST_DESC",
      };
      ...
      return this.getGenericSelect({ genericSqlPayload: selectPayload })
        .then((res: any) => {
          ...
          if (res.table_data) {
            ...
            //   if (item.ACTIVITY_ID == localThis.selectedProcessId) {
            //     itemsAlreadyInserted.push(item);
            //   }
            // });
            res.table_data.forEach((item: any) => {
              if (
                localThis.InsertedShoppingElement != undefined &&
                localThis.InsertedShoppingElement.filter(
                  (itemL: any) => itemL.PMS_SHOPPING_LIST_ID == item.PMS_SHOPPING_LIST_ID
                ).length == 0
              ) {
                //   if (
                ...
                //     ).length == 0
                //   ) {
                //     localThis.shoppinglistList.push(item);
                //   }
                // }
                localThis.shoppinglistList.push(item);
              }
  ...
  watch: {
    ...
    //     this.$confirm(msg).then((res: any) => {
    //       if (res) {
    //         localThis.updateValue(to[0].PMS_SHOPPING_LIST_ID);
    //       }
    //     });
