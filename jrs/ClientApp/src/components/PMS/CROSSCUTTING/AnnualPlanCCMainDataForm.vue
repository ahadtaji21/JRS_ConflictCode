          ...
          <v-row>
            <v-col :cols="$vuetify.breakpoint.xsOnly ? 12 : 6">
              <jrs-multi-select-table
                v-model="formData.CC"
                :label="$t('datasource.pms.annual-plan.objectives.c-c')"
                :dataSource="'PMS_VI_CONSTRUCTION_DEF_DIMENSION_CC'"
...
export default Vue.extend({
  ...
  methods: {
    ...
    save: function () {
      ...
      this.$confirm(msg).then((res) => {
        if (res) {
          let i: number = 0;
          for (i = 0; i < localThis.formData.CC.length; i++) {
            let payLoadD: any = {};
            payLoadD.ANNUAL_PLAN_ID = localThis.annual_plan_id;
            payLoadD.CC_ID = localThis.formData.CC[i].PMS_CONSTRUCTION_DEF_ID;
            payLoadD.PMS_CC_KEY_ID = localThis.formData.selectedCCKeyId;

            let savePayload: GenericSqlPayload = {
              spName: "PMS_SP_INS_UPD_ANNUAL_PLAN_CC",
    ...
    resetOverall() {
      ...
      localThis.formData.CC = [];
      localThis.dataSourceCondition =
        "PMS_DIMENSION='Principles, values, criteria, cross cutting'";
  ...
  beforeMount() {
    ...
    for (i = 0; i < localThis.alreadyInserted.length; i++) {
      cond += localThis.alreadyInserted[i].PMS_CC_ID;
      cond += ",";
    }
    cond += "-2";
    localThis.dataSourceCondition =
      "(PMS_DIMENSION='Principles, values, criteria, cross cutting') AND PMS_CONSTRUCTION_DEF_ID NOT IN (" +
      cond +
      ")";
  },
});
