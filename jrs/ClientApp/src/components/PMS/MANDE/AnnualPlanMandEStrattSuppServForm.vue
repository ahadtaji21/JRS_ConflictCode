          ...
          >
            <template v-slot:top>
              <div class="d-inline-flex align-center primary lighten-2 jrs-table-top">
                <h4>{{ $t("datasource.pms.annual-plan.objectives.s-s-s") }}</h4>
              ...
              <tr v-for="(item, index) in items" :key="item.ID">
                <td :class="'descClass'">
                  <v-checkbox
                    ...
                    @change="checkboxUpdated($event, index, item.ID, item.PMS_SSS_KEY_ID)"
                  ></v-checkbox>
                </td>
                <td :class="'descClass'">
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                      <span v-on="on">{{ item.PMS_STRAT_SUPP_SERV_DESC | subStr }}</span>
                    </template>
                    <span>{{ item.PMS_STRAT_SUPP_SERV_DESC }}</span>
                  </v-tooltip>
                </td>
                <td :class="'descClass'">
...
import Indicators from "../STRATSUPPSERV/AnnualPlanSSSIndicatorForm.vue";
...
*/
    checkboxUpdated(newValue: any, index: number, rel_id: number, key_id: number) {
      let localThis: any = this as any;
      ...
      for (i = 0; i < count; i++) {
        ...
        if (newValue) {
          localThis.formData.selectedSSSId = key_id; // localThis.stratsuppservList[index].PMS_SSS_KEY_ID;
    ...
    reloadSSS(item: any) {
      ...
      localThis.getAnnualPlanSSS();
    },
    getAnnualPlanSSS() {
      let localThis: any = this as any;
      localThis.selectedItem = null;
      localThis.showWait = true;
      localThis.stratsuppservList = [];
      let i: number = 0;
      let j: number = 0;
      let view: string = "PMS_VI_STRAT_SUPP_SERV_M_AND_E";
      let wherecond: string = `PMS_AP_ID = ${localThis.selectedAnnualPlanId}`;
      if (localThis.versioned > 0) {
        view = "PMS_VI_STRAT_SUPP_SERV_M_AND_E_VER";
        ...
        (res: any) => {
          ...
          if (res.table_data) {
            ...
            res.table_data.forEach((item: any) => {
              localThis.stratsuppservList.push(item);
            });
          }
    ...
    setupHeaders() {
      ...
      let tmp: JrsHeader[] = [
        ...
        {
          text: this.$i18n.t("datasource.pms.annual-plan.objectives.s-s-s").toString(),
