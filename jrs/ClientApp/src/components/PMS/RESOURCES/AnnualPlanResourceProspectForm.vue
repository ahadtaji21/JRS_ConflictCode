          ...
          >
            <template v-slot:top>
              <div class="d-inline-flex align-center primary lighten-2 jrs-table-top">
                <h4>{{ $t("datasource.pms.annual-plan.processes") }}</h4>
            ...
            <template v-slot:body="{ items }">
              <tr v-for="(item, index) in items" :key="item.ACT_ID">
                <td :class="'descClass'">
                  <v-checkbox
                    ...
                    @change="checkboxUpdated($event, index, item.ACT_ID)"
                  ></v-checkbox>
                </td>
                <td :class="'descClass'">
                  <v-tooltip bottom>
...
import BudgetList from "../SHOPPINGLIST/BudgetResourceListForm.vue";
...
*/
    checkboxUpdated(newValue: any, index: number, rel_id: number) {
      ...
      for (i = 0; i < count; i++) {
        ...
        localThis.showBudgetList = newValue;
        if (newValue) {
    ...
    getAnnualPlanObjectives() {
      ...
      let i: number = 0;
      let j: number = 0;
      let view: string = "PMS_VI_ANNUAL_PLAN_PROCESS_PROSPECT";
      let wherecond: string = `AP_ID = ${localThis.selectedAnnualPlanId} `;
      if (localThis.versioned > 0) {
        view = "PMS_VI_ANNUAL_PLAN_PROCESS_PROSPECT_VER";
      ...
      let selectPayload: GenericSqlSelectPayload = {
        ...
        orderStmt: "OBJ_ID,SRV_ID,ACT_ID,PMS_COI_ID,PMS_TOS_ID",
      };
