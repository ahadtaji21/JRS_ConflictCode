          ...
          <div class="d-inline-flex align-center primary lighten-2 jrs-table-top">
            ...
            >
              ...
              <search-table
                ...
                :selectedSSSId="formData.selectedSSSId"
                :indicatorsAlreadyAdded="indicatorList"
                @UpdateSSSIndicators="updateSSSIndicators"
        ...
        <template v-slot:body="{ items }">
          <tbody style="border: solid">
            <tr v-for="item in items" :key="item.ID" style="height: 10px">
              <td class="tablerow">{{ item.PMS_SSS_IND }}</td>
              <!-- <td class="tablerow">{{ item.PMS_SSS_IND_STANDARD }}</td>
              <td class="tablerow">{{ item.PMS_SSS_IND_PERIODICITY }}</td>
              <td class="tablerow">{{ item.PMS_SSS_IND_DISAGGREGATED }}</td> -->
              <td class="tablerow">{{ item.PMS_SSS_IND_TECHNIQUE }}</td>
              <td class="tablerow">{{ item.PMS_AP_SSS_END_DATE | formatDate }}</td>
...
import SearchTable from "./AnnualPlanSSSIndicatorSearch.vue";
...
/* localThis.localFormData.selectedProcessId = localThis.selectedProcessId;
      ...
      localThis.localFormData.selectedSSSId = localThis.selectedSSSId;
...
export default Vue.extend({
  ...
  methods: {
    ...
    closeOutpDialogAndSave() {
      ...
      this.$emit("reloadSSSList");
    },

    updateSSSIndicators(item: any) {
      let localThis: any = this as any;
    ...
    UpdateItems(item: any) {
      ...
      for (i = 0; i < item.length; i++) {
        ...
        payLoadD.PMS_SSS_ID = localThis.formData.selectedSSSId;
        payLoadD.PMS_IND_ID = item[i].PMS_SSS_IND_ID;
      ...
      localThis.showNewSnackbar({ typeName: "success", text: "Info Updated" }); // Feedback to user
    },
    getRnd() {
    ...
    getOutputIndicators() {
      ...
      let wherecond: string = `PMS_SSS_KEY_ID = ${localThis.formData.selectedSSSId}`;
    ...
    editIndicator(item: any) {
      ...
      localThis.localFormData.sssIndicatorRelId = item.PMS_SSS_IND_REL_ID;
      localThis.localFormData.sssId = localThis.formData.selectedSSSId;
      localThis.localFormData.sssIndicatorId = item.PMS_SSS_IND_ID;
      localThis.localFormData.QUESTION = item.PMS_SSS_IND_QUESTION;
      localThis.localFormData.INDICATOR = item.PMS_SSS_IND;
      localThis.localFormData.VALUE = item.VALUE;
      localThis.localFormData.END_DATE = item.PMS_AP_SSS_END_DATE;
      localThis.localFormData.STANDARD = item.PMS_SSS_IND_STANDARD;
      localThis.localFormData.TECHNIQUE = item.PMS_SSS_IND_TECHNIQUE;
      localThis.localFormData.DISAGGREGATED = item.PMS_SSS_IND_DISAGGREGATED;
      localThis.localFormData.PERIODICITY = item.PMS_SSS_IND_PERIODICITY;
      localThis.localFormData.VALUE = item.VALUE;
    ...
    deleteItem(item: any) {
      ...
      this.$confirm(msg).then((res: any) => {
        if (res) {
          let payLoadD = {} as any;
          payLoadD.PMS_SSS_IND_REL_ID = item.PMS_SSS_IND_REL_ID;

          let savePayload: GenericSqlPayload = {
            spName: "PMS_SP_DEL_SSS_INDICATOR",
            actionType: SqlActionType.NUMBER_0,
            jsonPayload: JSON.stringify(payLoadD),
          };
          localThis
  ...
  mounted() {
    ...
    if (localThis.formData.selectedSSSId > 0) {
      localThis.getOutputIndicators();
    }

    EventBus.$on("closeDialogeAndSaveSSSInd", (data: any) => {
      localThis.showIndicatorDetails = false;
      if (localThis.formData.selectedSSSId > 0) {
        localThis.editMode = false;
        localThis.getOutputIndicators();
