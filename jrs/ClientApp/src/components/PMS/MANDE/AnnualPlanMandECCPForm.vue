                ...
                <td :class="'descClass'">
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                      <span v-on="on">{{ item.PMS_CCP_DESC | subStr }}</span>
                    </template>
                    <span>{{ item.PMS_CCP_DESC }}</span>
                  </v-tooltip>
                </td>
                <td :class="'descClass'">
...
import Indicators from "../CROSSCUTTING/AnnualPlanCCPIndicatorForm.vue";
...
*/
    checkboxUpdated(newValue: any, index: number, rel_id: number) {
      ...
      for (i = 0; i < count; i++) {
        ...
        if (newValue) {
          localThis.formData.selectedCCPId = localThis.ccpList[index].PMS_CCP_KEY_ID;
