          ...
          <div class="d-inline-flex align-center primary lighten-2 jrs-table-top">
            ...
            >
              ...
              <search-table
                ...
                :selectedOvgId="formData.selectedOvgId"
                :indicatorsAlreadyAdded="indicatorList"
                :key="Math.ceil(Math.random() * 1000)"
                @UpdateOVGIndicators="updateOVGIndicators"
              ></search-table>
            </v-dialog>
        ...
        <template v-slot:body="{ items }">
          <tbody style="border: solid">
            <tr v-for="item in items" :key="item.ID" style="height: 10px">
              <td class="tablerow">{{ item.PMS_OVG_IND }}</td>
              <!-- <td class="tablerow">{{ item.STANDARD }}</td>
              <td class="tablerow">{{ item.PMS_OVG_IND_PERIODICITY }}</td>
              <td class="tablerow">{{ item.PMS_OVG_IND_DISAGGREGATED }}</td> -->
              <td class="tablerow">{{ item.PMS_OVG_IND_TECHNIQUE }}</td>
...
import SearchTable from "./AnnualPlanOverallGoalIndicatorSearch.vue";
import outpIndDet from "./AnnualPlanOverallGoalIndicatorMainDataForm.vue";
...
/* localThis.localFormData.selectedProcessId = localThis.selectedProcessId;
      ...
      localThis.localFormData.selectedOvgId = localThis.selectedOvgId;
...
export default Vue.extend({
  ...
  methods: {
    ...
    UpdateItems(item: any) {
      ...
      for (i = 0; i < item.length; i++) {
        payLoadD.PMS_OVG_IND_REL_ID = 0;
        payLoadD.PMS_OVG_ID = localThis.formData.selectedOvgId;
        payLoadD.PMS_IND_ID = item[i].PMS_OVG_IND_ID;

        let savePayload: GenericSqlPayload = {
          spName: "PMS_SP_INS_OVG_INDICATOR",
    ...
    getOutputIndicators() {
      ...
      let i: number = 0;
      let j: number = 0;
      let view: string = "PMS_VI_OVERALL_GOAL_INDICATOR";
      let wherecond: string = `PMS_OVG_KEY_ID = ${localThis.formData.selectedOvgId}`;
      if (localThis.versioned > 0) {
        view = "PMS_VI_OVERALL_GOAL_INDICATOR_VER";
          ...
          if (res.table_data) {
            ...
            res.table_data.forEach((item: any) => {
              if (item.STANDARD == undefined || item.STANDARD == "")
                item.STANDARD = item.PMS_OVG_IND_STANDARD;
    ...
    editIndicator(item: any) {
      ...
      localThis.localFormData.ovgIndicatorRelId = item.PMS_OVG_IND_REL_ID;
      localThis.localFormData.ovgId = localThis.formData.selectedOvgId;
      localThis.localFormData.ovgIndicatorId = item.PMS_OVG_IND_ID;
      localThis.localFormData.QUESTION = item.PMS_OVG_IND_QUESTION;
      localThis.localFormData.INDICATOR = item.PMS_OVG_IND;
      localThis.localFormData.END_DATE = item.END_DATE;
      localThis.localFormData.VALUE = item.VALUE;

      localThis.localFormData.STANDARD = item.STANDARD;
      localThis.localFormData.TECHNIQUE = item.PMS_OVG_IND_TECHNIQUE;
      localThis.localFormData.DISAGGREGATED = item.PMS_OVG_IND_DISAGGREGATED;
      localThis.localFormData.PERIODICITY = item.PMS_OVG_IND_PERIODICITY;
      localThis.localFormData.VALUE = item.VALUE;
    ...
    deleteItem(item: any) {
      ...
      this.$confirm(msg).then((res: any) => {
        if (res) {
          let payLoadD = {} as any;
          payLoadD.PMS_OVG_IND_REL_ID = item.PMS_OVG_IND_REL_ID;

          let savePayload: GenericSqlPayload = {
            spName: "PMS_SP_DEL_OVERALL_INDICATOR",
            actionType: SqlActionType.NUMBER_0,
            jsonPayload: JSON.stringify(payLoadD),
          };
          localThis
  ...
  mounted() {
    ...
    if (localThis.formData.selectedOvgId > 0) {
      localThis.getOutputIndicators();
    }

    EventBus.$on("closeDialogeAndSaveOvgInd", (data: any) => {
      localThis.showIndicatorDetails = false;
      if (localThis.formData.selectedOvgId > 0) {
        localThis.getOutputIndicators();
        this.$emit("reloadOvgList");
