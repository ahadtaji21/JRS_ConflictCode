<template>
  <v-row v-if="selectedObjectId != undefined && selectedObjectId != null">
    <v-col :cols="12">
      <v-toolbar style="background-color: #42a5f5">
        <v-spacer></v-spacer>

        <v-tooltip bottom v-if="isUpdatableForm">
          <template v-slot:activator="{ on }">
            <v-icon color="white" @click="saveNarrativeData" v-on="on"
              >mdi-content-save</v-icon
            >
          </template>
          <span>{{ $t("general.save-narrative-only") }}</span>
        </v-tooltip>

        <template v-slot:extension>
          <v-tabs v-model="activeTab" centered background-color="#42a5f5" dark>
...
export default Vue.extend({
  ...
  beforeDestroy() {
    ...
    EventBus.$off("saveGTNarrativeFromMainSave");
  ...
  mounted() {
    ...
    EventBus.$on("saveGTNarrativeFromMainSave", (data: any) => {
      if (localThis.narrativeChanged == true) {
        localThis.narrativeChanged = false;
        localThis.saveNarrativeData();
      }
