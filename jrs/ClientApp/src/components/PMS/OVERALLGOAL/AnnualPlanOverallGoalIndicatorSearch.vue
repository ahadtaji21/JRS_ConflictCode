          ...
          >
            <template v-slot:top>
              <div class="d-inline-flex align-center primary lighten-2 jrs-table-top">
                <h3>
                  {{ $t("datasource.pms.annual-plan.objectives.overall-indicators") }}
                </h3>
...
export default Vue.extend({
  ...
  methods: {
    ...
    updateValue(newVal: String) {
      (this as any).$emit("UpdateOVGIndicators", newVal);
    },
    setupHeaders() {
      let localThis: any = this as any;
      let tmp: JrsHeader[] = [
    ...
    getIndicators() {
      ...
      let selectPayload: GenericSqlSelectPayload = {
        viewName: "PMS_VI_OVG_INDICATOR",
        colList: null,
        whereCond: `PMS_OVG_KEY_ID = ${localThis.selectedOvgId}`, // AND IMS_LANGUAGE_LOCALE='${i18n.locale}'`,
              ...
              if (localThis.indicatorsAlreadyAdded != undefined) {
                if (
                  localThis.indicatorsAlreadyAdded.filter(
                    (itemL: any) => itemL.PMS_OVG_IND_ID == item.PMS_OVG_IND_ID
                  ).length == 0
                ) {
  ...
  watch: {
    ...
    //     localThis.formData.ovgId = localThis.selectedOvgId;
    //     localThis.formData.ovgIndicatorId = to[0].PMS_OVG_IND_ID;
    //     localThis.formData.QUESTION = to[0].PMS_OVG_IND_QUESTION;
    //     localThis.formData.INDICATOR = to[0].PMS_OVG_IND;
