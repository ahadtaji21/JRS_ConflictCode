          ...
          <v-row>
            <v-col :cols="$vuetify.breakpoint.xsOnly ? 12 : 6">
              <jrs-multi-select-table
                ...
                :label="$t('datasource.pms.annual-plan.objectives.c-c-p')"
                :dataSource="'PMS_VI_CONSTRUCTION_DEF_DIMENSION_CCP'"
...
export default Vue.extend({
  ...
  methods: {
    ...
    save: function () {
      ...
      this.$confirm(msg).then((res) => {
        if (res) {
          let i: number = 0;
          for (i = 0; i < localThis.formData.CCP.length; i++) {
            let payLoadD: any = {};
            payLoadD.ANNUAL_PLAN_ID = localThis.annual_plan_id;
            payLoadD.CCP_ID = localThis.formData.CCP[i].PMS_CONSTRUCTION_DEF_ID;
            payLoadD.PMS_CCP_KEY_ID = localThis.formData.selectedOvgId;

            let savePayload: GenericSqlPayload = {
              spName: "PMS_SP_INS_UPD_ANNUAL_PLAN_CCP",
    ...
    resetOverall() {
      ...
      localThis.formData.CCP = [];
      localThis.dataSourceCondition = "PMS_DIMENSION='Cross-cutting process'";
  ...
  beforeMount() {
    ...
    for (i = 0; i < localThis.alreadyInserted.length; i++) {
      cond += localThis.alreadyInserted[i].PMS_CCP_ID;
      cond += ",";
    }
    cond += "-2";
    localThis.dataSourceCondition =
      "PMS_DIMENSION='Cross-cutting process' AND PMS_CONSTRUCTION_DEF_ID NOT IN (" +
      cond +
      ")";
  },
});
