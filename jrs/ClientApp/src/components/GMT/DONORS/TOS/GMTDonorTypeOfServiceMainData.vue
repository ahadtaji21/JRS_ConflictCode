        ...
        <v-form v-model="formValid" ref="myForm" lazy-validation class="text-capitalize">
          ...
          <v-row>
            <v-col :cols="12">
              ...
              :counter="150"></v-text-field>-->
                <v-text-field
                :label="$t('pms.coi-description')"
                v-model="formData.GMT_TOS_DESCRIPTION"
                :counter="150"
                :rules="rules.required"
              ></v-text-field>
            </v-col>
          ...
          <v-row>
            <v-col :cols="$vuetify.breakpoint.xsOnly ? 12 : 6">
              <v-autocomplete
                v-model="formData.GMT_JRS_TOS_ID"
                :items="jrsTOS"
                outlined
                :loading="isLoading"
            ...
            <v-col :cols="$vuetify.breakpoint.xsOnly ? 12 : 6">
              <v-checkbox :label="$t('pms.coi-enabled')" v-model="formData.GMT_TOS_ENABLED"></v-checkbox>
            </v-col>
          </v-row>
        </v-form>
      </v-container>
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      tableName: "GMT_DONOR_TYPE_OF_SERVICE",
      idObject: 0,
      formValid: false,
  ...
  beforeMount() {
    ...
    localThis.formValid = false;
     localThis.rstValidation();
    localThis.idObject = localThis.formData.GMT_TOS_ID;
    if (localThis.jrsTOS.length == undefined || localThis.jrsTOS.length == 0)
  ...
  methods: {
    ...
    getListCond() {
      ...
      let selectPayload: GenericSqlSelectPayload = {
        viewName: "GMT_VI_DONORS_JRS_TYPE_OF_SERVICE",
        colList: null,
        whereCond:
          localThis.formData.GMT_JRS_TOS_ID == undefined
            ? `GMT_DONOR_ID is null or txt not in (select txt from  GMT_VI_DONORS_JRS_TYPE_OF_SERVICE where GMT_DONOR_ID = ${localThis.selectedObjectId})`
            : `vlue=${localThis.formData.GMT_JRS_TOS_ID} OR (GMT_DONOR_ID is null or txt not in (select txt from  GMT_VI_DONORS_JRS_TYPE_OF_SERVICE where GMT_DONOR_ID = ${localThis.selectedObjectId}))`,
        orderStmt: "txt",
      };
        ...
        .then((res: any) => {
          ...
          if (res.table_data) {
            ...
            res.table_data.forEach((item: any) => {
              ...
              itm.value = item.vlue + "";
              if (
                localThis.jrsTOS.filter(
                  (itemL: any) => itemL.value == itm.value
                ).length == 0
              ) {
                localThis.jrsTOS.push(itm);
              }
            });
          }
        })
    ...
    save() {
      ...
      let msgAdd: string = this.$i18n
        .t("datasource.gmt.donor.tos.add-tos")
        .toString();

      let id = 0;
      let msg = msgUpd;

      if (localThis.formData.GMT_TOS_ID == undefined) {
        msg = msgAdd;
        localThis.formData.GMT_TOS_ID = 0;
      }
      let obj: any;
      ...
      this.$confirm(msg).then((res) => {
        if (res) {
          let savePayload: GenericSqlPayload = {
            spName: "GMT_SP_INS_UPD_TYPE_OF_SERVICE",
