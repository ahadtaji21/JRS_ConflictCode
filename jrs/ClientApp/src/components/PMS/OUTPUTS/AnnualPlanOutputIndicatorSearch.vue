          ...
          >
            <template v-slot:top>
              <div class="d-inline-flex align-center primary lighten-2 jrs-table-top">
                <h3>
                  {{ $t("datasource.pms.annual-plan.objectives.output-indicators") }}
                </h3>
    ...
    <!-- <v-dialog
                ...
                >
                  <pms-outp-ind-det-form
                    ...
                    @closeDialogeAndSaveOutpInd="closeDialogAndSave"
                    :formData="formData"
                    :selectedOutpId="selectedOutId"
                  ></pms-outp-ind-det-form>
                </v-dialog> -->
  </div>
</template>
...
export default Vue.extend({
  ...
  methods: {
    ...
    updateValue(newVal: String) {
      (this as any).$emit("UpdateOutputIndicators", newVal);
    },
    save() {
    ...
    getIndicators() {
      ...
      let selectPayload: GenericSqlSelectPayload = {
        viewName: "PMS_VI_OUTCOME_OUTPUT_INDICATOR",
        colList: "PMS_OUTP_IND,PMS_OUTP_IND_ID,PMS_OBJ_IND_TECHNIQUE",
        whereCond: `PMS_OUTP_ID = ${localThis.selectedOutId}`, // AND IMS_LANGUAGE_LOCALE='${i18n.locale}'`,
        orderStmt: "PMS_OUTP_IND_QUESTION",
      };
              ...
              if (localThis.indicatorsAlreadyAdded != undefined) {
                if (
                  localThis.indicatorsAlreadyAdded.filter(
                    (itemL: any) => itemL.PMS_OUTP_IND_ID == item.PMS_OUTP_IND_ID
                  ).length == 0
                ) {
  ...
  watch: {
    ...
    //   localThis.formData.procOutpRelId = localThis.selectedProcOutRelId;
    //   localThis.formData.outpIndicatorRelId = 0;
    //   localThis.formData.outpIndicatorId=to[0].PMS_OUTP_IND_ID;
    //   localThis.formData.QUESTION =to[0].PMS_OUTP_IND_QUESTION;
    //   localThis.formData.INDICATOR=to[0].PMS_OUTP_IND;
