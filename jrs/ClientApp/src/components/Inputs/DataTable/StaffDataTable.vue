<template>
    <div>
        <div class="header">
            ...
            <excel-generate-report :withoutHeader="withoutHeader"
        ...
        <div class="table-container">
            <table class="table">
                <thead>
                    <tr>
                        <th v-for="col in columns.filter((ele) => !ele.hide)"
                            :key="col.id"
                            :style="{ width: col.width ? `${col.width}px` : '220px' }">
                            <div class="thdiv">
                                <p>{{ $t(col.label) }}</p>
                                <input v-if="!col.actions"
                ...
                <tbody v-if="tableData.length > 0">
                    <tr v-for="(d, idx) in filteredData" :key="idx">
                        <td v-for="(col, idx) in columns.filter((ele) => !ele.hide)"
                            ...
                            <span v-if="!col.actions && !col.field.toLowerCase().endsWith('_is_active')" class="table-cell-p">
                                {{ d[col.field] }}
                            </span>
            ...
            inputChange(search_query, search_key) {
                ...
                if (this.searchPagination) {
                    if (search_query && search_query.length > 1) {
                        console.log("1")
                        const searchBefore = this.searchQuery.find((ele) => ele.search_key == search_key)
                        if (searchBefore) searchBefore.search_query = search_query;
                        else this.searchQuery.push({ search_key, search_query });
                        console.log("1:1: searchbefore: ", searchBefore);
                    } else {
                        this.searchQuery = this.searchQuery.filter(ele => ele.search_key != search_key);
                        console.log("2: this.searchQuery: ", this.searchQuery);
                    }
