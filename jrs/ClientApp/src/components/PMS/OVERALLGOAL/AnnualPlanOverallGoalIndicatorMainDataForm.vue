...
export default Vue.extend({
  ...
  methods: {
    ...
    save: function () {
      ...
      if (
        localThis.localFormData.ovgIndicatorRelId == undefined ||
        localThis.localFormData.ovgIndicatorRelId == 0
      ) {
        msg = this.$i18n
    ...
    updateItem(end_date: Date) {
      ...
      let localThis = this as any;
      let payLoadD = {} as any;
      payLoadD.PMS_OVG_ID = localThis.localFormData.ovgId;
      payLoadD.PMS_IND_ID = localThis.localFormData.ovgIndicatorId;
      payLoadD.STANDARD = localThis.localFormData.STANDARD;
      payLoadD.VALUE = localThis.localFormData.VALUE;
      payLoadD.END_DATE = end_date;
      if (localThis.localFormData.ovgIndicatorRelId == undefined)
        payLoadD.PMS_OVG_IND_REL_ID = 0;
      else payLoadD.PMS_OVG_IND_REL_ID = localThis.localFormData.ovgIndicatorRelId;
      ...
      localThis
        ...
        .then((res: any) => {
          ...
          EventBus.$emit("closeDialogeAndSaveOvgInd", null);
        })
        .catch((err: any) => {
          localThis.showNewSnackbar({
            typeName: "error",
  ...
  // beforeDestroy() {
  //   EventBus.$off("closeDialogeAndSaveOvgInd");
  // },
  filters: {
