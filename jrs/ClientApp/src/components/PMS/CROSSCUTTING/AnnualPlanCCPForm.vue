      ...
      >
        <template v-slot:top>
          <div class="d-inline-flex align-center primary lighten-2 jrs-table-top">
            <h3>{{ $t("datasource.pms.annual-plan.objectives.c-c-p") }}</h3>
            <v-spacer></v-spacer>
            <span v-if="!onlyReadLocal">
              ...
              >
                ...
                <ap-ccp
                  ...
                  @closeDialogCCP="closeDialogCCP"
                  @saveDialoge="saveDialoge"
                  :versioned="versioned"
                  :key="Math.ceil(Math.random() * 1000)"
                  :alreadyInserted="ccpList"
                ></ap-ccp>
              </v-dialog>
            </span>
            <v-spacer></v-spacer>
        ...
        <template v-slot:body="{ items }">
          <tbody style="border: solid">
            <tr v-for="item in items" :key="item.PMS_CCP_ID" style="height: 10px">
              <!-- <td class="tablerow">{{ item.PMS_CCP_TARGET_DESC }}</td> -->
              <td class="tablerow">{{ item.PMS_CCP_DESC }}</td>
              <td style="text-align: center">
                <v-tooltip bottom>
                  <template v-slot:activator="{ on }">
                    <v-icon small class="mr-2" @click="editCCP(item)" v-on="on"
                      >mdi-circle-edit-outline</v-icon
                    >
                  </template>

                  <span>{{
                    $t("datasource.pms.annual-plan.objectives.ccp-details")
                  }}</span>
                </v-tooltip>
                ...
                <v-tooltip bottom v-if="!onlyReadLocal">
                  ...
                  <span>{{
                    $t("datasource.pms.annual-plan.objectives.delete-ccp")
                  }}</span>
                </v-tooltip>
              </td>
            </tr>
...
import AnnualCCP from "./AnnualPlanCCPMainDataForm.vue";
...
export default Vue.extend({
  ...
  methods: {
    ...
    closeDialogCCP(item: String) {
      let localThis: any = this as any;
      localThis.editMode = false;
    },
    closeCCPDialog(item: string) {
      let localThis: any = this as any;
    ...
    newCCP() {
      ...
      localThis.formData.CCP = [] as any;
      localThis.formData.CCP[0] = {} as any;
    ...
    closeCCPDialogReload(item: string) {
      let localThis: any = this as any;
      localThis.showOutpDetails = false;
      EventBus.$emit("showActTabs");
      localThis.reloadItems(null);
    ...
    editCCP(item: any) {
      let localThis = this as any;
      localThis.formData = {};
      localThis.formData.selectedOvgId = item.PMS_CCP_KEY_ID;
      localThis.formData.selectedAnnualPlanId = localThis.selectedAnnualPlanId;
      // localThis.formData.PMS_TARGET_ID_STR = item.PMS_TARGET_ID + "";
      localThis.formData.CCP = [] as any;
      localThis.formData.CCP[0] = {} as any;
      localThis.formData.CCP[0].PMS_CONSTRUCTION_DEF_ID = item.PMS_CCP_ID;
    ...
    deleteItem(item: any) {
      ...
      this.$confirm(msg).then((res: any) => {
        if (res) {
          let payLoadD = {} as any;
          payLoadD.PMS_CCP_KEY_ID = item.PMS_CCP_KEY_ID;

          let savePayload: GenericSqlPayload = {
            spName: "PMS_SP_DEL_CCP",
