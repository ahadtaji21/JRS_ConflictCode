      ...
      >
        <template v-slot:top>
          <div class="d-inline-flex align-center primary lighten-2 jrs-table-top">
            <h3>{{ $t("datasource.pms.annual-plan.objectives.c-c-s") }}</h3>
            <v-spacer></v-spacer>
            <span v-if="!onlyReadLocal">
              ...
              >
                ...
                <ap-ccs
                  ...
                  @closeDialogCCS="closeDialogCCS"
                  @saveDialoge="saveDialoge"
                  :versioned="versioned"
                  :key="Math.ceil(Math.random() * 1000)"
                  :alreadyInserted="ccsList"
                ></ap-ccs>
              </v-dialog>
            </span>
            <v-spacer></v-spacer>
        ...
        <template v-slot:body="{ items }">
          <tbody style="border: solid">
            <tr v-for="item in items" :key="item.PMS_CCS_ID" style="height: 10px">
              <!-- <td class="tablerow">{{ item.PMS_CCS_TARGET_DESC }}</td> -->
              <td class="tablerow">{{ item.PMS_CCS_DESC }}</td>
              <td style="text-align: center">
                <v-tooltip bottom>
                  <template v-slot:activator="{ on }">
                    <v-icon small class="mr-2" @click="editCCS(item)" v-on="on"
                      >mdi-circle-edit-outline</v-icon
                    >
                  </template>

                  <span>{{
                    $t("datasource.pms.annual-plan.objectives.ccs-details")
                  }}</span>
                </v-tooltip>

                <v-tooltip bottom v-if="!onlyReadLocal">
                  <template v-slot:activator="{ on }">
                  ...
                  <span>{{
                    $t("datasource.pms.annual-plan.objectives.delete-ccs")
                  }}</span>
                </v-tooltip>
              </td>
            </tr>
...
import AnnualCCS from "./AnnualPlanCCSMainDataForm.vue";
...
export default Vue.extend({
  ...
  methods: {
    ...
    closeDialogCCS(item: String) {
      let localThis: any = this as any;
      localThis.editMode = false;
    },
    closeCCSDialog(item: string) {
      let localThis: any = this as any;
    ...
    saveDialoge() {
      let localThis: any = this as any;
      localThis.editMode = false;
      localThis.crossCuttingServiceDialogReload();
    },
    newCCS() {
      let localThis: any = this as any;
      localThis.formData = {};
      localThis.formData.selectedAnnualPlanId = localThis.selectedAnnualPlanId;
      localThis.formData.CCS = [] as any;
      localThis.formData.CCS[0] = {} as any;
      localThis.editMode = true;
    },

    crossCuttingServiceDialogReload(item: string) {
      let localThis: any = this as any;
      //localThis.showOutpDetails = false;
      EventBus.$emit("showActTabs");
      localThis.reloadItems(null);
    },
    closeCCSDialogReload(item: string) {
      let localThis: any = this as any;
      localThis.showOutpDetails = false;
      EventBus.$emit("showActTabs");
      localThis.reloadItems(null);
    ...
    editCCS(item: any) {
      let localThis = this as any;
      localThis.formData = {};
      localThis.formData.selectedOvgId = item.PMS_CCS_KEY_ID;
      localThis.formData.selectedAnnualPlanId = localThis.selectedAnnualPlanId;
      // localThis.formData.PMS_TARGET_ID_STR = item.PMS_TARGET_ID + "";
      localThis.formData.CCS = [] as any;
      localThis.formData.CCS[0] = {} as any;
      localThis.formData.CCS[0].PMS_CONSTRUCTION_DEF_ID = item.PMS_CCS_ID;
    ...
    deleteItem(item: any) {
      ...
      this.$confirm(msg).then((res: any) => {
        if (res) {
          let payLoadD = {} as any;
          payLoadD.PMS_CCS_KEY_ID = item.PMS_CCS_KEY_ID;

          let savePayload: GenericSqlPayload = {
            spName: "PMS_SP_DEL_CCS",
