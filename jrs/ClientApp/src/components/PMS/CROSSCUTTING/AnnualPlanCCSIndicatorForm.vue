  ...
  <div>
          ...
          <div class="d-inline-flex align-center primary lighten-2 jrs-table-top">
            ...
            >
              ...
              <search-table
                ...
                :selectedCCSId="formData.selectedCCSId"
        ...
        <template v-slot:body="{ items }">
          <tbody style="border: solid">
            <tr v-for="item in items" :key="item.ID" style="height: 10px">
              <td class="tablerow">{{ item.PMS_CCS_IND }}</td>
              <td class="tablerow">{{ item.PMS_CCS_IND_STANDARD }}</td>
              <td class="tablerow">{{ item.PMS_CCS_IND_PERIODICITY }}</td>
              <td class="tablerow">{{ item.PMS_CCS_IND_DISAGGREGATED }}</td>
              <td class="tablerow">{{ item.PMS_CCS_IND_TECHNIQUE }}</td>
              <td class="tablerow">{{ item.PMS_AP_CCS_END_DATE | formatDate }}</td>
    ...
    >
      <pms-outp-ind-det-form
        @closeCCSDetDialoge="closeOutpDialog"
        @closeDialogeAndSaveCCSInd="closeOutpDialogAndSave"
...
import SearchTable from "./AnnualPlanCCSIndicatorSearch.vue";
import outpIndDet from "./AnnualPlanCCSIndicatorMainDataForm.vue";
...
/* localThis.localFormData.selectedProcessId = localThis.selectedProcessId;
      ...
      localThis.localFormData.selectedCCSId = localThis.selectedCCSId;
...
export default Vue.extend({
  ...
  methods: {
    ...
    getOutputIndicators() {
      ...
      let i: number = 0;
      let j: number = 0;
      let view: string = "PMS_VI_CCS_INDICATOR";
      let wherecond: string = `PMS_CCS_KEY_ID = ${localThis.formData.selectedCCSId}`;
      if (localThis.versioned > 0) {
        view = "PMS_VI_CCS_INDICATOR_VER";
    ...
    editIndicator(item: any) {
      ...
      localThis.localFormData.ccsIndicatorRelId = item.PMS_CCS_IND_REL_ID;
      localThis.localFormData.ccsId = localThis.formData.selectedCCSId;
      localThis.localFormData.ccsIndicatorId = item.PMS_CCS_IND_ID;
      localThis.localFormData.QUESTION = item.PMS_CCS_IND_QUESTION;
      localThis.localFormData.INDICATOR = item.PMS_CCS_IND;
      localThis.localFormData.VALUE = item.VALUE;
      localThis.localFormData.END_DATE = item.PMS_AP_CCS_END_DATE;
      localThis.localFormData.STANDARD = item.PMS_CCS_IND_STANDARD;
      localThis.localFormData.TECHNIQUE = item.PMS_CCS_IND_TECHNIQUE;
      localThis.localFormData.DISAGGREGATED = item.PMS_CCS_IND_DISAGGREGATED;
      localThis.localFormData.PERIODICITY = item.PMS_CCS_IND_PERIODICITY;
      localThis.localFormData.VALUE = item.VALUE;
    ...
    deleteItem(item: any) {
      ...
      this.$confirm(msg).then((res: any) => {
        if (res) {
          let payLoadD = {} as any;
          payLoadD.PMS_CCS_IND_REL_ID = item.PMS_CCS_IND_REL_ID;

          let savePayload: GenericSqlPayload = {
            spName: "PMS_SP_DEL_CCS_INDICATOR",
  ...
  mounted() {
    ...
    if (localThis.formData.selectedCCSId > 0) {
      localThis.getOutputIndicators();
    }

    EventBus.$on("closeDialogeAndSaveCCSInd", (data: any) => {
      localThis.showIndicatorDetails = false;
      if (localThis.formData.selectedCCSId > 0) {
        localThis.editMode = false;
        localThis.getOutputIndicators();
